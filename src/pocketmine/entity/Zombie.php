...
class Zombie extends Monster{
	...
	private $moveTicker = 0;
	private $hate_r = 16;
	private $attack_r = 1.5;
	private $fire_r = 1.3;
	private $hateTicker = 0;
	...
	public function onUpdate($currentTick){
		...
		if($this->isAlive()){
			...
			if(($time >= 0 and $time < 14000) or $time >= 23000) $this->setOnFire(2);
			...
			if(!$p) {
				...
				$this->hated = $p;
				if($p->distance($this) <= $this->fire_r) $p->setOnFire(2);
				if(!$this->tempTicking){
			...
			if($this->motionY == 0) $this->motionY -= $this->gravity;
